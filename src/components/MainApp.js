...
const MainApp = ({ initialTab }) => {
  ...
  const getInitialTab = () => {
    if (userRole === 'trainer') return 4; // Attendance tab
    if (initialTab === 'attendance') return 4; // Attendance tab
    return 0; // Default to fee dashboard tab
  };
  ...
  const handleTabSwitch = async (newTab) => {
    ...
    // Refresh data when switching to Fee Dashboard (tab 0)
    if (newTab === 0) {
      try {
        setLoading(true);
  ...
  const handleAddStudent = async (studentData) => {
    try {
      ...
      // Automatically create fee record for the new student for the current month only
      const currentDate = new Date();
      const currentMonthStr = format(currentDate, 'yyyy-MM-dd'); // Use full date format for current month
      
      const feeData = {
        studentId: createdStudent.id, // Use the ID from the created student
        feesMonth: currentMonthStr,
        amount: 0,
        status: 'Unpaid',
        paymentDate: null,
        paymentMode: null
      ...
      try {
        ...
        console.error('Error creating fee record for current month:', feeError);
        // Don't fail the entire student creation if fee creation fails
      }
      ...
      setSnackbar({ open: true, message: 'Student added successfully with current month fee record', severity: 'success' });
    } catch (error) {
      console.error('Error adding student:', error);
          ...
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            ...
            <Typography variant="h6" noWrap component="div">
              {userRole === 'trainer' ? 'Attendance Management' : 
                selectedTab === 0 ? 'Fee Dashboard' : 
                selectedTab === 1 ? 'Students' :
                selectedTab === 2 ? 'Batches' :
                selectedTab === 3 ? 'Exports' :
                selectedTab === 4 ? 'Attendance' : 'Fee Dashboard'}
            </Typography>
          </Box>
